...
const pricingMatrix: Record<string, Record<string, { price: number; duration: number }>> = {
  '1 BHK': {
...
const DomesticHelpForm = ({ isOpen, onClose }: DomesticHelpFormProps) => {
  ...
  const selectedPricing = houseSize && floors ? pricingMatrix[houseSize]?.[floors] : null;

  // Calculate bathroom price if selected
  const bathroomPrice = bathroom?.bathrooms 
    ? bathroomPricing[bathroom.bathrooms as keyof typeof bathroomPricing]?.price || 300
    : 300;

  // Calculate dusting price if selected
  const dustingPrice = dusting?.duration
    ? dustingPricing[dusting.duration as keyof typeof dustingPricing]?.price || 800
    : 800;

  // Calculate dishwashing price if selected
  const dishwashingPrice = dishwashing?.peopleCount
    ? dishwashingPricing[dishwashing.peopleCount as keyof typeof dishwashingPricing]?.price || 400
    : 400;
  ...
  const calculateTotalDuration = (): number => {
    ...
    if (selectedServices.includes('bathroom') && bathroom?.bathrooms) {
      totalMinutes += bathroomPricing[bathroom.bathrooms as keyof typeof bathroomPricing]?.duration || 0;
    }
    
    if (selectedServices.includes('dusting') && dusting?.duration) {
      totalMinutes += dustingPricing[dusting.duration as keyof typeof dustingPricing]?.duration * 60 || 0;
    }
    
    if (selectedServices.includes('dishwashing') && dishwashing?.peopleCount) {
      totalMinutes += dishwashingPricing[dishwashing.peopleCount as keyof typeof dishwashingPricing]?.duration || 0;
    }
  ...
  if (showSummary) {
    ...
    const serviceDetails = {
      ...
      details: `House Size: ${houseSize}, Floors: ${floors}${bathroom ? `, Bathrooms: ${bathroom.bathrooms}` : ''}${dusting ? `, Dusting Duration: ${dusting.duration}` : ''}${dishwashing ? `, Dishwashing People: ${dishwashing.peopleCount}` : ''}`
    };
    ...
    return (
      <BathroomCleaningForm 
        ...
        onClose={(formData?: { bathrooms: string }) => {
          ...
          if (formData) {
            dispatch(updateDomesticHelpServices([...selectedServices, 'bathroom']));
    ...
    return (
      <DustingForm 
        ...
        onClose={(formData?: { duration: string }) => {
          ...
          if (formData) {
            dispatch(updateDomesticHelpServices([...selectedServices, 'dusting']));
    ...
    return (
      <DishwashingAddOn
        ...
        onClose={(formData?: { peopleCount: string; frequency: string }) => {
          ...
          if (formData) {
            dispatch(updateDomesticHelpServices([...selectedServices, 'dishwashing']));
            ...
            <div className="space-y-8">
              ...
              <div className="bg-white rounded-lg p-4">
                ...
                <p className="text-gray-600 mb-4">
                  Daily Working Hours ~ {formatDuration(calculateTotalDuration())}
                </p>
                <select
                  value={workShift}
                  onChange={(e) => dispatch(updateDomesticHelpSchedule({ workShift: e.target.value }))}
              ...
              <div className="bg-white rounded-lg p-4">
                ...
                <input
                  ...
                  value={startDate}
                  onChange={(e) => dispatch(updateDomesticHelpSchedule({ startDate: e.target.value }))}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500"
                />
              </div>
              ...
              <div className="bg-white rounded-lg p-4">
                ...
                <textarea
                  value={notes}
                  onChange={(e) => dispatch(updateDomesticHelpNotes(e.target.value))}
                  placeholder="Notes.."
                  rows={4}
                  className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-yellow-500"
                />
              ...
              <div className="bg-white rounded-lg p-4">
                ...
                <label className="flex items-center space-x-3">
                  <input
                    ...
                    onChange={(e) => dispatch(updateDomesticHelpPreferences({ hasReligiousPreference: e.target.checked }))}
              ...
              <div className="bg-white rounded-lg p-4">
                <label className="flex items-start space-x-3">
                  <input
                    ...
                    onChange={(e) => dispatch(updateDomesticHelpPreferences({ agreeToTerms: e.target.checked }))}
              ...
              <div className="flex justify-between items-center mb-2">
                <div>
                  ...
                  <div className="text-gray-600">
                    Daily Working Hours ~{formatDuration(calculateTotalDuration())} approx.
                  </div>
                </div>
                <button
  ...
  if (showBroomingDetails) {
    return (
      <div className="fixed inset-0 z-50 overflow-y-auto">
        <div className="fixed inset-0 bg-black/50" onClick={() => setShowBroomingDetails(false)}></div>
          ...
          <div className="relative bg-white w-full max-w-2xl rounded-2xl shadow-xl p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-12">
                {/* House Size */}
                <div>
                  <h3 className="text-xl font-semibold mb-2">Select house size <span className="text-sm font-normal text-gray-500">(Please describe the area of ONLY 1 floor)</span></h3>
                  <p className="text-gray-500 mb-4">Select 1 out of 6 options</p>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {['1 BHK', '2 BHK', '3 BHK', '4 BHK', '5 BHK', '6 BHK'].map((size) => (
                      <button
                        key={size}
                        type="button"
                        onClick={() => dispatch(updateDomesticHelpHouseDetails({ houseSize: size }))}
                ...
                {/* Number of Floors */}
                <div>
                  <h3 className="text-xl font-semibold mb-2">How many such floors?</h3>
                  <p className="text-gray-500 mb-4">Select 1 out of 5 options</p>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    {['1 floor only', '2 floors', '3 floors', '4 floors', '5 floors'].map((option) => (
                      <button
                        key={option}
                        type="button"
                        onClick={() => dispatch(updateDomesticHelpHouseDetails({ floors: option }))}
                        className={`py-3 px-4 rounded-xl border text-center ${
                          floors === option
                            ? 'border-yellow-500 bg-yellow-50 text-yellow-700'
                            : 'border-gray-200 text-gray-600 hover:border-yellow-200'
                ...
                {/* Dogs Question */}
                <div>
                  <h3 className="text-xl font-semibold mb-2">Do you have dog(s)?</h3>
                  <p className="text-gray-500 mb-2">Select 1 out of 2 options</p>
                  <p className="text-gray-400 text-sm mb-4">This would help us search for a pet friendly worker (if needed)</p>
                  <div className="flex gap-4">
                    {['yes', 'no'].map((option) => (
                      <button
                        key={option}
                        type="button"
                        onClick={() => dispatch(updateDomesticHelpDogs(option as 'yes' | 'no'))}
              ...
              <div className="mt-8 border-t border-gray-200 pt-4">
                <div className="flex justify-between items-center mb-2">
                  <div>
                    <div className="text-gray-600">
                      Monthly Salary ~₹{selectedPricing ? selectedPricing.price.toLocaleString() : ' 0.00'} approx.
                    </div>
                    <div className="text-gray-600">
                      Daily Working Hours ~{selectedPricing ? formatDuration(selectedPricing.duration) : '0:00'} approx.
                    </div>
                  </div>
                  <button
                    type="button"
      ...
      <div className="relative min-h-screen flex items-center justify-center p-4">
        <div className="relative bg-white w-full max-w-2xl rounded-2xl shadow-xl p-6">
          ...
          <div className="mb-8">
            ...
            <p className="text-yellow-500 mb-4">Brooming, Mopping service is mandatory</p>
            ...
            <div className="space-y-4">
              {services.map(service => {
                ...
                if (service.id === 'brooming' && selectedPricing) {
                  ...
                  dynamicPrice = bathroomPricing[bathroom.bathrooms as keyof typeof bathroomPricing]?.price || 300;
                } else if (service.id === 'dusting' && dusting?.duration) {
                  dynamicPrice = dustingPricing[dusting.duration as keyof typeof dustingPricing]?.price || 800;
                } else if (service.id === 'dishwashing' && dishwashing?.peopleCount) {
                  dynamicPrice = dishwashingPricing[dishwashing.peopleCount as keyof typeof dishwashingPricing]?.price || 400;
                }
                  ...
                  <div key={service.id} className="bg-white rounded-xl border border-gray-200 p-4">
                    <div className="flex items-start">
                      ...
                      <div className="ml-4 flex-1">
                        ...
                        <div className="text-yellow-500 font-medium">₹{dynamicPrice.toLocaleString()}/month <span className="text-gray-500 text-sm">starting</span></div>
